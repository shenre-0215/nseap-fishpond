"""第二组消融实验 —— 分别移除悬置层和闭环检测，量化每个组件贡献。

对比：
- NSEAP（完整）
- NSEAP-NoSusp（无悬置层）
- NSEAP-NoClose（无闭环检测）
- NSEAP-Static（无动态生长，第一组已做）
"""
...
from agents.nseap_ablations import NSEAPNoSuspensionAgent, NSEAPNoClosureAgent
